...
const AddSevas = () => {
  ...
  const handleRecurringSelect = (id) => {
    ...
    if (selected) {
      ...
      setSevaName("");
      // don’t reset date → allow manual entry
    }
    
  };

  // ------------------- useEffect -------------------
  useEffect(() => { fetchSevas(); }, [fetchSevas]);

  useEffect(() => {
    if (sevaType === "Event-Specific Sevas") fetchRecurringEvents();
  }, [sevaType, fetchRecurringEvents]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (sevaType === "General Sevas") {
        formData.append(
          "days",
          `${startDay}${startDay && endDay ? " to " : ""}${endDay}`
        );
      ...
      console.error("Save seva error:", err.response?.data || err.message);
      showMessage("Failed to save seva", "error");
    }
  };
...
{sevaType === "Event-Specific Sevas" && (
  <div className="form-group-seva">
    ...
    <input
      type="date"
      value={date}
      onChange={(e) => setDate(e.target.value)} // manual input enabled
      className="input-seva"
    />
  </div>
)}
